...
Class Guru_model extends CI_Model{
    ...
    {
            $query = $this->db->select('kode')->group_by('kode')->order_by('kode','asc')->get('tb_guru');
            return $query->result();
    }

    public function get_by_id($id)
    {
            $query = $this->db->where('id',$id)->get('tb_guru');
            return $query->row();
    }
    public function get_by_ni($no_induk)
    {
            $query = $this->db->where('no_induk',$no_induk)->get('tb_guru');
            return $query->row();
    }
    public function get_kode_1a(){
        $query = $this->db->where('kode','1a')->get('tb_guru');
        return $query->num_rows();
    }
    public function get_kode_1b(){
        $query = $this->db->where('kode','1b')->get('tb_guru');
        return $query->num_rows();
    }
	public function get_jabatan(){
		$query = $this->db->group_by('jabatan_new')->get('tb_guru');
		return $query->result();
	}
	public function count_by_jenis($jabatan,$jenis){
		$query = $this->db->where('jabatan_new',$jabatan)->where('jenis_kelamin',$jenis)->get('tb_guru');
		return $query->num_rows();
	}
    ...
    public function verifikasi_siswa(){
        ...
        $query = $this->db->where(['no_induk'=>$id_santri,'kode'=>$kode])->get('tb_guru');
        if($query->num_rows() > 0){
            return true;
        }else{
            return false;
    ...
    public function insert(){
        ...
        if($this->db->insert('tb_guru',$this)){
            return true;
        }else{
            return false;
        }
	...
	public function update_siswa_detail(){
		...
		if($update){
			...
			$update = $this->db->update('tb_guru',$data2,array('no_induk' => $this->input->post('no_induk')));
			return true;
		}else{
			return false;
		}
    ...
    public function update(){
        ...
        if($this->db->update('tb_guru', $this, array('id' => $_POST['id']))){
            return true;
        }else{
            return false;
        }
    ...
    {
        if($this->db->delete('tb_guru', array('id' => $id))){
            return true;
        }else{
            return false;
        }
